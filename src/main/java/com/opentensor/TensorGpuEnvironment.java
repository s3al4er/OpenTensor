...
/**
 * Tensor GPU component environment, exposed to computers as {@code gpu}.
 *
 * <p>Implements the same Lua API as the stock OpenComputers graphics card
 * ({@code bind}, {@code set}, {@code fill}, {@code copy}, {@code bitblt},
 * VRAM buffers, resolution/depth management, ...), with fixed datacenter
 * specs: 190x60 resolution, 8-bit color, Tier-3-class call budget scaled by
 * the card, and VRAM well above a stock Tier 3 card
 * ({@code totalMemory} = 11400 cells * {@code vramScreens}).</p>
 *
 * <p>Integration with OpenComputers happens exclusively through the public
 * driver API: the environment is instantiated by {@link TensorDriver} and
 * registered via {@code li.cil.oc.api.Driver}. Off-screen VRAM pages reuse
 * OC's text buffer utilities as plain library classes.</p>
 */
public final class TensorGpuEnvironment extends AbstractManagedEnvironment implements DeviceInfo {
    /** Maximum resolution of Tensor cards (matches OC Tier 3). */
    ...
    public static final int MAX_CELLS = MAX_WIDTH * MAX_HEIGHT; // 11400
    /** Maximum color depth of Tensor cards (matches OC Tier 3). */
    public static final TextBuffer.ColorDepth MAX_DEPTH = TextBuffer.ColorDepth.EightBit;
    /** Color depth in bits, for tooltips. */
    public static final int MAX_DEPTH_BITS = 8;

    // Tier-3-class call budget costs, multiplied by the card's cost scale.
    private static final double COST_SET_BACKGROUND = 1.0 / 256;
    private static final double COST_SET_FOREGROUND = 1.0 / 256;
    private static final double COST_SET_PALETTE_COLOR = 1.0 / 32;
    private static final double COST_SET = 1.0 / 512;
    private static final double COST_COPY = 1.0 / 128;
    private static final double COST_FILL = 1.0 / 256;
    // OC default bitbltCost (0.5) * 2^3 for Tier-3-class throughput.
    private static final double BITBLT_BASE = 4.0;
    ...
    // = 800) and multiplied by the card's cost scale. The raw config values
    // must NOT be used directly: a single full-screen fill would otherwise
    // cost thousands of energy (far beyond the 500-unit computer buffer,
    // creative case included) and silently fail with "not enough energy".
    private static final double ENERGY_SET = 2.0 / 800;
    private static final double ENERGY_FILL = 1.0 / 800;
    private static final double ENERGY_CLEAR = 0.1 / 800;
    private static final double ENERGY_COPY = 0.25 / 800;
    ...
    public TensorGpuEnvironment(TensorGpuSpec spec) {
        this(spec, null);
    }

    public TensorGpuEnvironment(TensorGpuSpec spec, EnvironmentHost host) {
        this.spec = spec;
    ...
    private String activeMissingMessage() {
        return bufferIndex == RESERVED_SCREEN_INDEX ? "no screen" : "invalid buffer index";
    }
    ...
    private double bitbltBudgetCost(TextBuffer dst, TextBuffer src) {
        if (src instanceof GpuTextBuffer page) {
            ...
            if (page.dirty()) {
                return BITBLT_BASE * costScale * (src.getWidth() * src.getHeight()) / (double) MAX_CELLS;
            }
            return 0.001;
        }
        return 0.0;
    ...
    private double bitbltEnergyCost(TextBuffer dst) {
        ...
        return ENERGY_COPY / 15 * costScale;
    }
    ...
    private String clockPart(double baseCost) {
        return String.valueOf(((long) (2000 / (baseCost * costScale))) / 100);
    }
    ...
    public Map<String, String> getDeviceInfo() {
        ...
        info.put(DeviceAttribute.Vendor, spec.vendor());
    ...
    public Object[] bitblt(Context context, Arguments args) throws Exception {
        ...
        synchronized (dst) {
            ...
            synchronized (src) {
                ...
                double tierCredit = 2.0; // (tier + 1) * 0.5 with Tier-3-class tier.
    ...
    public Object[] setBackground(Context context, Arguments args) throws Exception {
        ...
        if (bufferIndex == RESERVED_SCREEN_INDEX) {
            context.consumeCallBudget(COST_SET_BACKGROUND * costScale);
        }
    ...
    public Object[] setForeground(Context context, Arguments args) throws Exception {
        int color = args.checkInteger(0);
        if (bufferIndex == RESERVED_SCREEN_INDEX) {
            context.consumeCallBudget(COST_SET_FOREGROUND * costScale);
        }
        return onActiveBuffer(screen -> {
    ...
    public Object[] setPaletteColor(Context context, Arguments args) throws Exception {
        ...
        if (bufferIndex == RESERVED_SCREEN_INDEX) {
            context.consumeCallBudget(COST_SET_PALETTE_COLOR * costScale);
            context.pause(0.1);
        }
        return onActiveBuffer(screen -> {
            try {
        ...
        if (vram != null && node.address() != null) {
            ...
            for (int i = 0; i < pages.size(); i++) {
                try {
                    ...
                    TensorPageHolder pageHolder = TensorPageHolder.fromTag(page.getCompound(TAG_PAGE_DATA));
                    if (pageHolder == null) {
                        continue;
                    }
    ...
    public void saveData(MutableDataComponentHolder holder) {
        ...
        for (int id : ids) {
            try {
                ...
                TensorPageHolder pageHolder = new TensorPageHolder();
    ...
    /**
     * Per-page component holder used to capture an off-screen VRAM buffer via
     * the buffer's own component persistence, then freeze it into NBT with
     * plain ops (text data is registry-free; anything else degrades to the
     * page being skipped instead of failing the save).
     */
    static final class TensorPageHolder extends MutableNbtComponentHolder {
        TensorPageHolder() {
            super();
        }
        ...
        static TensorPageHolder fromTag(CompoundTag tag) {
            TensorPageHolder holder = new TensorPageHolder();
            DataComponentPatch.CODEC.parse(NbtOps.INSTANCE, tag).resultOrPartial().ifPresent(holder.components::applyPatch);
            return holder;
        }
    }
}
